...
#PER WEEK
df_week_test = df_test
df_week_test['datetime'] = df_test['datetime']
df_week_test = df_test.set_index('datetime', drop=False).groupby([pd.Grouper(key='datetime',freq='D')])['value'].mean().reset_index()

#PER WEEK
df_week_train = df_train
df_week_train['datetime'] = df_train['datetime']
df_week_train = df_train.set_index('datetime', drop=False).groupby([pd.Grouper(key='datetime',freq='D')])['value'].mean().reset_index()

df_test = df_week_test
df_train = df_week_train
